...
KeyboardServoPub::KeyboardServoPub(rclcpp::Node::SharedPtr &node)
  ...
  if (joint_command_in_topic_.rfind("~/", 0) == 0) {
    joint_command_in_topic_ =
        "/servo_server/" +
        joint_command_in_topic_.substr(2, cartesian_command_in_topic_.length());
  }
  ...
  servo_start_client_ =
      node_->create_client<std_srvs::srv::Trigger>("/servo_server/start_servo");
...
template <typename T>
void KeyboardServoPub::_declare_or_get_param(T &output_value,
  ...
  try {
    ...
    RCLCPP_WARN_STREAM(node_->get_logger(), "InvalidParameterTypeException("
                                                << param_name << "): " << e.what());
...
void KeyboardServoPub::keyLoop() {
  ...
  puts(
      "Use 'W' to Cartesian jog in the world frame, and 'E' for the End-Effector "
      "frame");
...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<rclcpp::Node> node =
      rclcpp::Node::make_shared("xarm_moveit_servo_keyboard_node", node_options);
