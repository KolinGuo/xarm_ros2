...
def launch_setup(context, *args, **kwargs):
    ...
    node_executable = LaunchConfiguration(
        "node_executable", default="xarm_planner_node"
    )
    ...
    if (
        robot_type.perform(context) != "lite"
        and add_gripper.perform(context) in ("True", "true")
        and use_gripper_node.perform(context) in ("True", "true")
    ):
        planning_group = (
            "uf850_gripper"
            if robot_type.perform(context) == "uf850"
            else "xarm_gripper"
        )
