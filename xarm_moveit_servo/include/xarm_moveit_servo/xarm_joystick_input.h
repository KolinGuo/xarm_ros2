...
class JoyToServoPub : public rclcpp::Node {
 ...
 private:
  template <typename T>
  void _declare_or_get_param(T &output_value, const std::string &param_name,
                             const T default_value = T {});
  bool _convert_xbox360_joy_to_cmd(
      const std::vector<float> &axes, const std::vector<int> &buttons,
      std::unique_ptr<geometry_msgs::msg::TwistStamped> &twist,
      std::unique_ptr<control_msgs::msg::JointJog> &joint);
  bool _convert_spacemouse_wireless_joy_to_cmd(
      const std::vector<float> &axes, const std::vector<int> &buttons,
      std::unique_ptr<geometry_msgs::msg::TwistStamped> &twist);

  void _filter_twist_msg(std::unique_ptr<geometry_msgs::msg::TwistStamped> &twist,
                         double val = 0.05);
