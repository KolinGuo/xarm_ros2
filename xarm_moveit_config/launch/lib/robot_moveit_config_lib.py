...
def add_prefix_to_moveit_params(
    ...
    if controllers_yaml:
        for i, name in enumerate(controllers_yaml["controller_names"]):
            ...
            if name in controllers_yaml:
                controllers_yaml["{}{}".format(prefix, name)] = controllers_yaml.pop(
                    name
                )
    if ompl_planning_yaml:
        for name in list(ompl_planning_yaml.keys()):
            if name != "planner_configs":
                ompl_planning_yaml["{}{}".format(prefix, name)] = (
                    ompl_planning_yaml.pop(name)
                )
    if kinematics_yaml:
    ...
    if joint_limits_yaml:
        for name in list(joint_limits_yaml["joint_limits"]):
            joint_limits_yaml["joint_limits"]["{}{}".format(prefix, name)] = (
                joint_limits_yaml["joint_limits"].pop(name)
            )
...
def get_xarm_robot_description_parameters(
    ...
    urdf_arguments["ros2_control_plugin"] = urdf_arguments.get(
    ...
    return {
        "robot_description": get_xacro_file_content(
            xacro_file=xacro_urdf_file, arguments=urdf_arguments
        ),
        "robot_description_semantic": get_xacro_file_content(
            xacro_file=xacro_srdf_file, arguments=srdf_arguments
        ),
