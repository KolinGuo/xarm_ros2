...
enum SPACEMOUSE_WIRELESS_BUTTON { SPM_BTN_LEFT = 0, SPM_BTN_RIGHT = 1 };

JoyToServoPub::JoyToServoPub(const rclcpp::NodeOptions &options)
    : Node("joy_to_twist_publisher", options),
      dof_(7),
      ros_queue_size_(10),
      joystick_type_(JOYSTICK_XBOX360_WIRED),
  ...
  _declare_or_get_param<std::string>(cartesian_command_in_topic_,
                                     "moveit_servo.cartesian_command_in_topic",
                                     cartesian_command_in_topic_);
  _declare_or_get_param<std::string>(joint_command_in_topic_,
                                     "moveit_servo.joint_command_in_topic",
                                     joint_command_in_topic_);
  _declare_or_get_param<std::string>(robot_link_command_frame_,
                                     "moveit_servo.robot_link_command_frame",
                                     robot_link_command_frame_);
  _declare_or_get_param<std::string>(ee_frame_name_, "moveit_servo.ee_frame_name",
                                     ee_frame_name_);
  _declare_or_get_param<std::string>(planning_frame_, "moveit_servo.planning_frame",
                                     planning_frame_);

  if (cartesian_command_in_topic_.rfind("~/", 0) == 0) {
    cartesian_command_in_topic_ =
        "/servo_server/" +
        cartesian_command_in_topic_.substr(2, cartesian_command_in_topic_.length());
  }
  if (joint_command_in_topic_.rfind("~/", 0) == 0) {
    joint_command_in_topic_ =
        "/servo_server/" +
        joint_command_in_topic_.substr(2, joint_command_in_topic_.length());
  }

  // Setup pub/sub
  joy_sub_ = this->create_subscription<sensor_msgs::msg::Joy>(
      joy_topic_, ros_queue_size_,
      std::bind(&JoyToServoPub::_joy_callback, this, std::placeholders::_1));

  twist_pub_ = this->create_publisher<geometry_msgs::msg::TwistStamped>(
      cartesian_command_in_topic_, ros_queue_size_);
  joint_pub_ = this->create_publisher<control_msgs::msg::JointJog>(
      joint_command_in_topic_, ros_queue_size_);
  // collision_pub_ =
  // this->create_publisher<moveit_msgs::msg::PlanningScene>("/planning_scene",
  // 10);

  // Create a service client to start the ServoServer
  servo_start_client_ =
      this->create_client<std_srvs::srv::Trigger>("/servo_server/start_servo");
...
template <typename T>
void JoyToServoPub::_declare_or_get_param(T &output_value,
  ...
  try {
    ...
    RCLCPP_WARN_STREAM(this->get_logger(), "InvalidParameterTypeException("
                                               << param_name << "): " << e.what());
    RCLCPP_ERROR_STREAM(this->get_logger(),
                        "Error getting parameter \'"
                            << param_name << "\', check parameter type in YAML file");
    throw e;
  }
...
void JoyToServoPub::_filter_twist_msg(
    std::unique_ptr<geometry_msgs::msg::TwistStamped> &twist, double val) {
...
bool JoyToServoPub::_convert_xbox360_joy_to_cmd(
  ...
  } else if (buttons[XBOX360_BTN_START] &&
             planning_frame_ == robot_link_command_frame_) {
    planning_frame_ = ee_frame_name_;
  }

  if (buttons[XBOX360_BTN_A] || buttons[XBOX360_BTN_B] || buttons[XBOX360_BTN_X] ||
      buttons[XBOX360_BTN_Y] || axes[cross_key_lr] || axes[cross_key_fb]) {
...
bool JoyToServoPub::_convert_spacemouse_wireless_joy_to_cmd(
    const std::vector<float> &axes, const std::vector<int> &buttons,
    std::unique_ptr<geometry_msgs::msg::TwistStamped> &twist) {
...
void JoyToServoPub::_joy_callback(const sensor_msgs::msg::Joy::SharedPtr msg) {
  ...
  switch (joystick_type_) {
    ...
    case JOYSTICK_XBOX360_WIRELESS:  // xbox360 wireless
      if (msg->axes.size() != 8 || msg->buttons.size() != 11) return;
      pub_twist =
          _convert_xbox360_joy_to_cmd(msg->axes, msg->buttons, twist_msg, joint_msg);
      break;
    case JOYSTICK_SPACEMOUSE_WIRELESS:  // spacemouse wireless
      if (msg->axes.size() != 6 || msg->buttons.size() != 2) return;
      pub_twist =
          _convert_spacemouse_wireless_joy_to_cmd(msg->axes, msg->buttons, twist_msg);
      break;
    default:
      return;
  }
  if (pub_twist) {
    ...
    // RCLCPP_INFO(this->get_logger(), "linear=[%f, %f, %f], angular=[%f, %f,
    // %f]",
