...
def add_prefix_to_ros2_control_params(prefix, ros2_control_params):
    ...
    for name in list(ros2_control_params.keys()):
        ...
        controller_manager_ros__parameters = ros2_control_params.get(
            "controller_manager", {}
        ).get("ros__parameters", {})
        if name in controller_manager_ros__parameters:
            controller_manager_ros__parameters[new_name] = (
                controller_manager_ros__parameters.pop(name)
            )


def generate_ros2_control_params_temp_file(
    ...
    if ros_namespace or prefix or add_gripper or add_bio_gripper or update_rate:
        ...
        if update_rate is not None:
            ros2_control_params_yaml["controller_manager"]["ros__parameters"][
                "update_rate"
            ] = update_rate
        if add_gripper:
            gripper_control_params_path = os.path.join(
                get_package_share_directory("xarm_controller"),
                "config",
                "{}_gripper_controllers.yaml".format(robot_type),
            )
            ...
            if os.path.exists(gripper_control_params_path):
                ...
                for name, value in gripper_control_params_yaml["controller_manager"][
                    "ros__parameters"
                ].items():
                    ros2_control_params_yaml["controller_manager"]["ros__parameters"][
                        name
                    ...
                    if name in gripper_control_params_yaml:
                        ros2_control_params_yaml[name] = gripper_control_params_yaml[
                            name
                        ]
...
def generate_dual_ros2_control_params_temp_file(
    ros2_control_params_path_1,
    ros2_control_params_path_2,
    prefix_1="L_",
    prefix_2="R_",
    add_gripper_1=False,
    add_gripper_2=False,
    ...
    if add_gripper_1:
        gripper_control_params_path = os.path.join(
            get_package_share_directory("xarm_controller"),
            "config",
            "{}_gripper_controllers.yaml".format(robot_type_1),
        )
        # check file is exists or not
        if os.path.exists(gripper_control_params_path):
            with open(gripper_control_params_path, "r") as f:
    ...
    if add_gripper_2:
        gripper_control_params_path = os.path.join(
            get_package_share_directory("xarm_controller"),
            "config",
            "{}_gripper_controllers.yaml".format(robot_type_2),
        )
        # check file is exists or not
        if os.path.exists(gripper_control_params_path):
            with open(gripper_control_params_path, "r") as f:
